...
@ccclass
export default class HookShop extends cc.Component {
        ...
        for (let i = 0; i < commoditys.length; i++) {
            for (let j = 0; j < pur_commodity_ids.length; j++) {
                if (commoditys[i].name === pur_commodity_ids[j]) {
                    ...
                    let skin_id = Cache.GetCache(CacheType.Hook_SkinId);
                    if (commoditys[i].name === skin_id) {
                        ...
                        this.Start.Role.UpdateHookSkin(skin_id);
                    } else {
    ...
    ButtonClick(lv: any, click: string) {
        ...
        switch (click) {
            ...
            case "yes":
                ...
                //缓存已购买商品ID
                Cache.SetCache(CacheType.Pur_Hook, pur_str);
            case "no":
        ...
        if (isneedbuy) {
            ...
            this.Start.Role.UpdateHookSkin(click);
            Cache.SetCache(CacheType.Hook_SkinId, click);
        }
    }
