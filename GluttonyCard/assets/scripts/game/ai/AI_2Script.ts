...
export default class AI_2 extends AI {
    constructor(scriptmanage: ScriptManage) {
        super(scriptmanage);
    }
    //随机自身卡牌
    RanSelfCard() {
        let ran = Math.floor(Math.random() * this.ScriptManage.GameManage.Player_3_Cards_Array.length);
        this.Play(this.ScriptManage.GameManage.Player_3_Cards_Array[ran]);
    }
    //获取自身卡牌
    GetSelfCard() {
        //是否出牌成功
    ...
    Play(card: cc.Node) {
        ...
        card.runAction(cc.sequence(cc.moveTo(0.3, node_pos), cc.callFunc(() => {
            ...
            //卡牌下标索引
            let ind = this.ScriptManage.GameManage.Player_3_Cards_Array.indexOf(card);
            //从手牌中移除
            this.ScriptManage.GameManage.Player_3_Cards_Array.splice(ind, 1);
        })));
    }
    //获取最大点数的花色
    GetMaxCountSuit(): boolean {
        ...
        if (max_suit === 4) {
            //获取方块
            let blocks_node = this.GetBlocks(this.ScriptManage.GameManage.Player_3_Cards_Array);
            if (blocks_node) {
                //出牌
        ...
        //获取可吃卡牌
        let iseat_node = this.GetIsEatCard(this.ScriptManage.GameManage.Player_3_Cards_Array, max_suit);
        if (iseat_node) {
            //出牌
