...
import Chessboard_1 from "./ChessboardScript _1";
...
@ccclass
export default class ArcadeScript extends cc.Component {
    ...
    //___________________预制体
    //___________________音效
    //道具使用音效
    @property({ url: cc.AudioClip })
    public Prop_Use: string = null;
    //背景音效
   ...
   // private Level_Num = 0;
    ...
    onLoad() {
        ...
        // }
        //获取棋盘管理
        this.ChessBoard = cc.find("/Canvas/Chessboard/Chessboard").getComponent(Chessboard_1);
        //初始化
        this.Init();
    }
    //初始化
    ...
    ArrayMoveLine() {
        ...
        for (let i = 0; i < moveline_pos.length; i++) {
            ...
            let start_y = this.ChessBoard.ArrayPos_y(start_x);
    ...
    // //关卡检测
    // LevelDetection() {
    //     cc.sys.localStorage.setItem("Level", this.Level_Num);
    //     for (let i = 0; i < this.LevelArray.length; i++) {
    ...
    PropsButton(lv: any, name: string) {
        //道具使用音效
        cc.audioEngine.play(this.Prop_Use, false, 1);
        if (this.Prop_Show) {
            if (name === "kinglet") {
