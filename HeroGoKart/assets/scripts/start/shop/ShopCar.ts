...
export default class NewClass extends cc.Component {
    ...
    private Car_Names: cc.SpriteFrame[] = [];
    /**
     * @property [Array]角色皮肤
     */
    @property([cc.SpriteFrame])
    private Car_Skins: cc.SpriteFrame[] = [];
    /**
     * @property 购买提示框
     */
     ...
     * @property 当前汽车ID
     */
    private Current_Car_ID: string = null;
    /**
     * @property 准备购买的汽车ID
     */
    ...
    Init() {
        ...
        this.Current_Car_ID = this.GetCurrentCarID();

        this.AddListenter()
        this.SetCommoditysStatus(this.Commoditys, this.HaveCommodityIDs, this.Current_Car_ID);
    ...
    private RemoveListenter() {
        //商城--->角色
        EventCenter.RemoveListenter(EventType.Coin_Amount, "ShopCar");
    }
     ...
     */
    private SetHintBoxYes(hint: cc.Node, car_skins: cc.SpriteFrame[], car_names: cc.SpriteFrame[], readyTobuy_ID: string) {
        //角色皮肤
        let car_skin: cc.SpriteFrame = null;
        for (let i = 0; i < car_skins.length; i++) {
            let car_skin_id = car_skins[i].name
            if (car_skin_id === readyTobuy_ID) {
                car_skin = car_skins[i];
                break;
            }
        }
        let car_sprite = hint.getChildByName("Box").getChildByName("car").getComponent(cc.Sprite);
        car_sprite.spriteFrame = car_skin;

        //角色名字
        let car_name: cc.SpriteFrame = null;
        for (let i = 0; i < car_names.length; i++) {
            let car_name_id = car_names[i].name
            let cha = car_name_id.charAt(0);
        ...
        name_sprite.spriteFrame = car_name;
    }
        ...
        for (let i = 0; i < this.Commoditys.length; i++) {
            ...
            if (commodity.name === click) {
                ...
                if (isBuy) {
                    ...
                    //选中商品
                    this.Current_Car_ID = this.SelectCommodity(this.Commoditys, click);
                    //更新角色皮肤
                }
                break;
            }
        ...
        for (let i = 0; i < commoditys.length; i++) {
            ...
            if (commodity.name === readyTobuy_id) {
                ...
                if (coin_value >= price_value) {
                    ...
                    Cache.SetCache(CacheType.HaveCommodity_CarIDs, have_str);
    ...
    private SelectCommodity(commoditys: cc.Node[], select_id: string): string {
        ...
        Cache.SetCache(CacheType.Current_Car_ID, select_id);
        return select_id;
    }
    ...
    private AddButton(commoditys: cc.Node[]) {
        for (let i = 0; i < commoditys.length; i++) {
            ...
            //绑定脚本文件名
            clickEventHandler.component = "ShopCar";
            //绑定按钮点击的执行函数
     ...
     */
    private SetCommoditysStatus(commoditys: cc.Node[], havecommodityIDs: string[], current_car_id: string) {
        for (let i = 0; i < commoditys.length; i++) {
            let commodity = commoditys[i];
            for (let j = 0; j < havecommodityIDs.length; j++) {
                ...
                if (commodity.name === havecommodityIDs[j]) {
                    let isopen = false;
                    if (commodity.name === current_car_id) {
                        isopen = true;
                    }
                    inUse.active = isopen;
                    coin.active = false;
     ...
     * @returns 当前角色ID
     */
    private GetCurrentCarID(): string {
        let id = Cache.GetCache(CacheType.Current_Car_ID);
        if (!id) {
            id = "1";
        }
    ...
    private GetHaveCommodityIDs(): string[] {
        let arr: string[] = [];
        let have_str = Cache.GetCache(CacheType.HaveCommodity_CarIDs);
